...
const currentlyWeatherResponse = [
    {
...
beforeEach(() => {
    ...
    fetch.mockResponse(async request => {
        ...
        if (request.url === `${process.env.REACT_APP_API_ENDPOINT_ACCUWEATHER}/locations/v1/cities/geoposition/search?apikey=${process.env.REACT_APP_API_KEY_ACCUWEATHER}&q=${latitude}%2C${longitude}`) {
            response = locationResponse;
        } else if (request.url === `${process.env.REACT_APP_API_ENDPOINT_ACCUWEATHER}/currentconditions/v1/${locationKey}?apikey=${process.env.REACT_APP_API_KEY_ACCUWEATHER}&details=true&language=${locale}`) {
            response = currentlyWeatherResponse;
        } else if (request.url === `${process.env.REACT_APP_API_ENDPOINT_ACCUWEATHER}/forecasts/v1/hourly/12hour/${locationKey}?apikey=${process.env.REACT_APP_API_KEY_ACCUWEATHER}&details=true&metric=true&language=${locale}`) {
            response = hourlyWeatherResponse;
        } else if (request.url === `${process.env.REACT_APP_API_ENDPOINT_ACCUWEATHER}/forecasts/v1/daily/5day/${locationKey}?apikey=${process.env.REACT_APP_API_KEY_ACCUWEATHER}&details=true&metric=true&language=${locale}`) {
...
describe('AccuWeatherApiClient', () => {
    it('getWeather', async () => {
        const weather = await AccuWeatherApiClient.getWeather(latitude, longitude, days, hours, locale);
